  ...
  return <>
    <section className="wk-overview-card" aria-label="Latest audit">
      ...
      <div className="wk-overview-card-main">
        <div className="wk-audit-artifact"><div className="wk-audit-artifact-top"><span className="wk-audit-artifact-mark"><FileText aria-hidden="true" /></span><span>{source?.mode === 'sample' ? 'Sample data' : 'Payment ledger'}</span></div><div className="wk-audit-artifact-center"><span className="wk-audit-artifact-label">PAYMENT REVIEW</span><strong>{auditLabel}</strong><div className="wk-audit-preview-rows">{env.records.slice(0, 2).map((record) => <div key={record.id}><span>{record.vendor}</span><span>{formatCurrency(record.amountPaid)}</span></div>)}</div><small>{env.records.length} records scanned · {all.length} findings surfaced</small></div><div className="wk-audit-artifact-foot"><span>Reclaim</span><span>{date}</span></div></div>
        <div className="wk-audit-details"><span className="wk-detail-eyebrow">Current audit</span><h3>{auditLabel}</h3><p>Review the evidence behind each finding, record your decision, and track confirmed recovery through its outcome.</p><dl><div><dt>Source</dt><dd>{source?.sourceLabel ?? 'Payment ledger'}</dd></div><div><dt>Records</dt><dd>{env.records.length} payments</dd></div><div><dt>Findings</dt><dd>{all.length} surfaced across {auditCount} {plural(auditCount, 'audit')}</dd></div><div><dt>Potential impact</dt><dd>{formatCurrency(l.potential)}</dd></div></dl></div>
      </div>
