 ...
 * only reads as the same thing if it keeps that arrangement. So nothing here
 ...
 * these screens animates; the launch screen's loop is still the only motion.
 */
...
/** One object per destination, so no two screens open the same way (§27). */
 ...
 *
 * Overview is absent on purpose: its object stands beside the value ladder at
...
export const SCREEN_BLURB: Partial<Record<WorkspaceMode, string>> = {
  ...
  recoveries: "Cases you've confirmed, and how far each one has got. Nothing reaches a vendor until you approve it.",
  vendors: 'Every vendor in this ledger, including the ones nothing was found against.',
  reports: 'What this ledger produced, stated plainly enough to forward to someone who has never opened Reclaim.',
  data: 'Which records Reclaim read, and what the columns in them made possible.',
  settings: 'Appearance, what this workspace is holding, and how to clear it.',
}
 ...
 * gives a case one plain-English cause — and one object — instead of a rule
