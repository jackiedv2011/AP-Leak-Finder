...
import { createDraftService, DraftRequestSchema, type DraftService } from './ai.ts'
...
import { entitlementsFor, isPlan, monthStart, PLAN_LIMITS, type Plan } from './plans.ts'
...
export function createApp(deps: AppDependencies): App {
  ...
  const auditsThisMonth = (userId: string) =>
    (db.prepare('SELECT COUNT(*) AS n FROM projects WHERE user_id = ? AND created_at >= ?').get(userId, monthStart()) as { n: number }).n
  const entitlements = (user: { id: string; plan: string }) => entitlementsFor(isPlan(user.plan) ? user.plan : 'free', auditsThisMonth(user.id))
  ...
  async function route(req: Request): Promise<Response> {
    ...
    if (path === '/api/account/upgrade' && method === 'POST') {
      requireUser(req)
      return json(501, { message: 'Upgrading to Pro is coming soon. No payment was taken.', code: 'coming_soon' })
    }
    ...
    if (path === '/api/dev/plan' && method === 'POST') {
      ...
      if (!isPlan(plan)) throw new HttpError(400, 'plan must be "free" or "pro".')
    ...
    if (projectMatch && method === 'PUT') {
      ...
      if (project.id !== projectMatch[1] || typeof project.name !== 'string' || typeof project.environment !== 'object') throw new HttpError(400, 'Malformed project.')
      ...
      if (isNew && !entitlements(user).canStartAudit) {
        const limit = PLAN_LIMITS[isPlan(user.plan) ? user.plan : 'free'].auditsPerMonth
    ...
    if (path === '/api/projects/import' && method === 'POST') {
      ...
      for (const project of projects) {
        if (typeof project.id !== 'string' || typeof project.name !== 'string' || typeof project.environment !== 'object') throw new HttpError(400, 'Malformed project.')
        const exists = db.prepare('SELECT 1 FROM projects WHERE user_id = ? AND id = ?').get(user.id, project.id)
        if (exists) {
    ...
    if (path === '/api/ai/draft' && method === 'POST') {
      ...
      const draft = await drafts.draft(parsed.data)
  ...
  const server = createServer(async (rawReq, rawRes) => {
    try {
      ...
      writeResponse(rawRes, response)
    } catch (err) {
      if (err instanceof HttpError) {
        writeResponse(rawRes, json(err.status, { message: err.message, code: err.code, fields: err.fields }))
        return
      }
      ...
      writeResponse(rawRes, json(500, { message: 'Something went wrong on our side.', code: 'server_error' }))
    }
  })
...
function upsertProject(db: DatabaseSync, userId: string, project: Record<string, unknown>): void {
  ...
  db.prepare(
    ...
    typeof project.createdAt === 'number' ? project.createdAt : now,
...
function serveStatic(root: string, urlPath: string): Response {
  ...
  for (const file of candidates) {
    ...
    if (existsSync(file) && statSync(file).isFile()) {
      return { status: 200, headers: { 'Content-Type': MIME[extname(file)] ?? 'application/octet-stream' }, body: readFileSync(file, 'utf8') }
    }
  }
  return empty(404)
}
